...
data "vsphere_network" "network" {
  name          = var.template.network != null ? var.template.network : var.common_vm_specs["vc_network"]
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
resource "vsphere_virtual_machine" "server" {
  ...
  disk {
    label            = "disk0"
    size             = var.template["diskSize"] != null ? var.template["diskSize"] :  data.vsphere_virtual_machine.template.disks.0.size
